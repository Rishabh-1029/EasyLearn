...
The platform will allow authenticated users to bring their own documents and web resources, combine them with reusable public knowledge, retrieve relevant evidence, and turn that evidence into grounded explanations, structured notes, and visual learning artifacts.
...
- Cheap and efficient embeddings
...
- Reuse publicly available knowledge efficiently.
...
- Select an embedding model based on quality, efficiency and cost.
...
- Automatically publish every user URL into the global store.
...
flowchart TD
    ...
    ING --> PARSE["Document / Web Parsing"]
    PARSE --> CHUNK["Chunking"]
    CHUNK --> META["Metadata"]
    META --> EMB["Embedding Model"]
    EMB --> STORE["Knowledge Store"]

    STORE --> RET["Retrieval Engine"]
    Q --> RET

    RET --> RANK["Optional Reranker"]
...
EasyLearn V2 will support three major knowledge acquisition modes.
...
The system should preserve useful metadata such as:
...
# 9. User-provided URLs

Users should be able to provide a URL and make its content part of their knowledge context.
...
A user-provided URL should be **private by default**, even when the underlying webpage is publicly accessible.

This is because the user explicitly selected the source as part of their personal knowledge workspace.

A future explicit action may allow:
...
```text
Publish to EasyLearn Knowledge
        ↓
GLOBAL
```
...
# 10. Web Search / Top-K Online Sources

EasyLearn should also provide a specialized capability for finding relevant online information.

The user does not necessarily need to provide a URL.
...
Search results should **not automatically become global knowledge**.
...
Answer
    ↓
Optional "Save Source"
    ↓
Global Knowledge Store
```

This prevents the global store from becoming polluted with every search result.

---

# 11. Two-Level Knowledge Store

The knowledge architecture has two logical scopes:
...
```mermaid
flowchart TD
    ROOT["Knowledge Store"] --> GLOBAL["Global Knowledge"]
    ROOT --> PRIVATE["Private Knowledge"]

    GLOBAL --> G1["Public Web Content"]
    GLOBAL --> G2["Published Resources"]
    GLOBAL --> G3["Reusable Public Articles"]

    PRIVATE --> U1["User Documents"]
    PRIVATE --> U2["User URLs"]
    PRIVATE --> U3["User-specific Knowledge"]
```

## 11.1 Global Store

Contains knowledge that can be reused across users.
...
Contains knowledge belonging to an individual user.
...
# 12. Privacy and Scope Enforcement
...
Embedding quality is one of the most important parts of the V2 system.
...
Retrieval is the core of V2.

The retrieval engine should find the smallest, most relevant evidence set that can support the user's request.

```mermaid
flowchart TD
    A["User Query"] --> B["Query Normalization"]
    B --> C["Scope-aware Retrieval"]
    C --> D["Candidate Chunks"]
    D --> E["Optional Reranking"]
    E --> F["Top Relevant Context"]
    F --> G["Context Assembly"]
```
...
3. Top-K candidate retrieval.
4. Optional reranking.
5. Context deduplication.
6. Citation metadata preservation.
...
These should be introduced only when real retrieval failures justify them.

---

# 18. Reranking Strategy
...
# 23. Complete Query-Time Pipeline
...
# 35. Performance & Resource Efficiency
...
# 42. Complete Ingestion-Time Pipeline
...
# 43. Complete User Query Pipeline
...
| D2  | Global + Private scopes               | Provides reusable public knowledge and private user knowledge |
...
| D5  | Benchmark embeddings                  | Retrieval quality is foundational                             |
...
# 47. Final Reference Architecture
...
flowchart TD
    ...
    RET --> RERANK["Optional Reranking"]
    RERANK --> CTX["Context Assembly"]
    CTX --> SUFF["Context Sufficiency"]

    SUFF -->|"Insufficient"| REFUSE["Grounded Limitation"]
    SUFF -->|"Sufficient"| ROUTER["LLM Router"]
...
> **EasyLearn V2 is a privacy-aware personal knowledge and learning platform where users can bring their own documents and web resources, combine them with reusable public knowledge, retrieve the most relevant evidence, and turn that evidence into grounded explanations, structured notes, and visual learning artifacts.**
...
# 49. Decisions Still Requiring Benchmarking
...
# 50. Final Strategy Summary

| Area         | Strategy                                                    |
| ------------ | ----------------------------------------------------------- |
| Product      | Personal knowledge + learning platform                      |
| Knowledge    | User-private + global public knowledge                      |
| Documents    | PDF, DOCX, TXT, MD                                          |
| Web          | Direct URL scraping + top-K online source retrieval         |
| Retrieval    | Scope-aware vector retrieval                                |
| Reranking    | Optional optimization                                       |
| Generation   | Three good-enough LLMs evaluated for quality/cost/latency   |
| Embeddings   | Cheap and efficient model selected through benchmarks       |
